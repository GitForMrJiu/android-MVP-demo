...
public class SettingsXmlParser {
	...
	static Map<String, Setting> parseSettings(Context context, String fileName) {
		...
		try {
			...
			Timber.d("read xml resource, filename = " + fileName);
			...
			while (eventType != XmlPullParser.END_DOCUMENT) {
				...
				case XmlPullParser.END_TAG:
					...
					if ("setting".equals(xmlResParser.getName())) {
						...
						String s =  gson.toJson(readSetting);
						Timber.d( String.format("parse new setting --->%s", s));
						readSetting = null;
					}

					if ("setting-array".equals(xmlResParser.getName())) {
						settingArray.add(readSettingArray);
						Timber.d( String.format("parse new settingArray --->%s", gson.toJson(readSettingArray)));
					...
					if ("extra".equals(xmlResParser.getName())) {
						...
						Timber.d( String.format("parse new settingExtra --->%s", gson.toJson(settingExtras)));
